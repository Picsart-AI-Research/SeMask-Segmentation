...
class SWSeMaskBlock(nn.Module):
    ...
    def __init__(self, dim, n_cls, window_size=7, num_sem_blocks=1, act_layer=nn.GELU, norm_layer=nn.LayerNorm):
        super().__init__()
        self.dim = dim
        self.window_size = window_size
        self.n_cls = n_cls
    ...
    def forward(self, x):
        ...
        for i in range(self.num_layers):
            ...
            if i in self.out_indices:
                ...
                cls_outs["res{}".format(i + 2)] = cls_out

        return outs, cls_outs
...
class D2SeMaskSwinTransformer(SeMaskSwinTransformer, Backbone):
    def __init__(self, cfg, input_shape):
        ...
        n_cls = cfg.MODEL.SEM_SEG_HEAD.NUM_CLASSES
        sem_window_size = cfg.MODEL.SWIN.SEM_WINDOW_SIZE
        num_sem_blocks = cfg.MODEL.SWIN.NUM_SEM_BLOCKS
    ...
    def forward(self, x):
        ...
        outputs = {}
        cls_outputs = {}
        y, cls_y = super().forward(x)
        for k in y.keys():
            if k in self._out_features:
                outputs[k] = y[k]
                cls_outputs[k] = cls_y[k]
